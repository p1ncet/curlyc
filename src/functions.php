...
if (!extension_loaded('curl')) {
	 ...
	 * @return resource
	 */
	function curl_copy_handle(Curl $ch) {
		return clone $ch;
	}
	 ...
	 */
	function curl_escape(Curl $ch, $str) {
	 ...
	 * "redirect_time"
	 */
	function curl_getinfo(Curl $ch, $opt = null) {
		$ch->is_init();
	 ...
	 * @param Curl $ch <p>A cURL handle returned by
	 * {@link www.php.net/manual/en/function.curl-init.php curl_init()}.</p>
	 */
	function curl_reset(Curl $ch) {
		$ch->reset();
	}
	 ...
	 */
	function curl_setopt(Curl $ch, $option, $value) {
		return $ch->setOpt($option, $value);
	}
	 ...
	 */
	function curl_setopt_array(Curl $ch, array $options) {
		return $ch->setOptArray($options);
	}
	...
	function curl_strerror($errornum) {
		$errors = [
			...
			CURLE_URL_MALFORMAT_USER          => "A requested feature, protocol or option was not found built-in in this libcurl due to a build-time decision.",
			...
			CURLE_FTP_ACCESS_DENIED           => "Access denied to remote resource",
			CURLE_FTP_USER_PASSWORD_INCORRECT => "FTP: The server failed to connect to data port",
			CURLE_FTP_WEIRD_PASS_REPLY        => "FTP: unknown PASS reply",
			CURLE_FTP_WEIRD_USER_REPLY        => "FTP: Accepting server connect has timed out",
			...
			CURLE_FTP_CANT_RECONNECT          => "Error in the HTTP2 framing layer",
			CURLE_FTP_COULDNT_SET_BINARY      => "FTP: couldn't set file type",
			CURLE_PARTIAL_FILE                => "Transferred a partial file",
			CURLE_FTP_COULDNT_RETR_FILE       => "FTP: couldn't retrieve (RETR failed) the specified file",
			CURLE_FTP_WRITE_ERROR             => "Unknown error",
			...
			CURLE_MALFORMAT_USER              => "Unknown error",
			...
			CURLE_FTP_COULDNT_SET_ASCII       => "Unknown error",
			CURLE_FTP_PORT_FAILED             => "FTP: command PORT failed",
			CURLE_FTP_COULDNT_USE_REST        => "FTP: command REST failed",
			CURLE_FTP_COULDNT_GET_SIZE        => "Unknown error",
			CURLE_HTTP_RANGE_ERROR            => "Requested range was not delivered by the server",
			...
			CURLE_BAD_CALLING_ORDER           => "Unknown error",
			CURLE_HTTP_PORT_FAILED            => "Failed binding local connection end",
			CURLE_BAD_PASSWORD_ENTERED        => "Unknown error",
			...
			CURLE_OBSOLETE                    => "Unknown error",
			CURLE_SSL_PEER_CERTIFICATE        => "SSL peer certificate or SSH remote key was not OK",
			...
			CURLE_SHARE_IN_USE                => "Unknown error",
		...
		return isset($errors[$errornum]) ? $errors[$errornum] : "Unknown error";
	}
	 ...
	 */
	function  curl_unescape(Curl $ch, $str) {
